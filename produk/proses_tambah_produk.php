...
if($_POST){
    ...
    if(empty($nama_produk)){
        ...
        echo "<script>alert('alamat tidak boleh kosong');location.href='tambah_produk.php';</script>";
    
    } elseif(empty($harga)){
        echo "<script>alert('harga tidak boleh kosong');location.href='tambah_produk.php';</script>";
   
    } elseif(empty($id_produk)){
        echo "<script>alert('id produk tidak boleh kosong');location.href='tambah_produk.php';</script>";
    
    } else {
        include "koneksi.php";
        $insert=mysqli_query($conn,"insert into produk (nama_produk, id_produk, deskripsi, harga) value ('".$nama_produk."','".$id_produk."','".$deskripsi."','".$harga."')") or die(mysqli_error($conn));
        if($insert){
